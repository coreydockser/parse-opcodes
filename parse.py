...
def main():
    ...
    #takes the index of ARGLUTS-START and ARGLUTS-END (and its opcode counterpart)

    # ensures that this function will still work even if the last item is not an empty string
        

    arg_start = opcode_formats.index('##ARGLUTS-START')
    arg_end = arg_start + opcode_formats.index('##ARGLUTS-END')
    arglut_list = opcode_formats[arg_start + 1 : arg_end + 1]

    opcode_start = opcode_formats.index('##OPCODES-START')
    opcode_end = opcode_start + opcode_formats.index('##OPCODES-END')
    opcodes = opcode_formats[opcode_start + 1 : opcode_end + 1]
    ...
    opcodes_list = []
    for item in opcodes:
        if not item == '' and not item.startswith(comment_flag):
    ...
    ## and opcode consists of a digit, followed by two periods a second digit, an equal sign, and a hex number
    def opcode_to_binary(opcode):
        ...
        if len(opcode_split)==2:
            opcode_out[2] = opcode_split[1]
            opcode_out[1] = opcode_split[0]
            opcode_out[0] = opcode_split[0]
        else:
            if opcode_split[2] != 'ignore':
                opcode_out[2] = bin(int(opcode_split[2], 0))[2:]
                opcode_out[1] = opcode_split[1]
                opcode_out[0] = opcode_split[0]
            else:
                opcode_out[2] = 'ignore'
                opcode_out[1] = opcode_split[1]
        ...
        for entry in opcode_set[1:]:
            ...
            if entry.startswith(numtup):
                ...
                if len(translated) == 3:
                    ...
                    first = int(translated[0])
                    opcode_len = 1
                    opcode_dict[first] = translated[1].zfill(opcode_len)
        ...
        if curr_cell.value != '%':
            ...
            if curr_cell.value.startswith(numtup):
                ...
                if curr_cell.value in argtup:
                    ...
                    for i in range(length_count + 1):
                        wsm.cell(column = stored_column + i, row = r).value = 'i[' + str(stored_tup[i]) + "]"
                        wsm.cell(column=stored_column + i, row=r).alignment = Alignment(horizontal='center')
                else:
                    for i in range(length_count + 1):
                        wsm.cell(column = stored_column + i, row = r).value = curr_cell.value
            #loops so long as dest_c + 1, which is the cell after the merge, isnt the last
            if dest_c + 1 < 34:
